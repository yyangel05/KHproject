...
public class ThemeManager {
	...
	public void insert(Theme theme) throws Exception { // 일반글 답변글 삽입
		Connection conn = null;
		...
		try {
			...
			if (theme.getParentId() == 0) {
				...
				rsGroup = stmtGroup.executeQuery("select max(GROUP_ID) from THEME_MESSAGE_PINK");
				int maxGroupId = 0;
				if (rsGroup.next()) {
				...
				pstmtOrder = conn.prepareStatement("select max(ORDER_NO) from THEME_MESSAGE_PINK "
			...
			if (theme.getOrderNo() > 0) {
				pstmtOrderUpdate = conn.prepareStatement("update THEME_MESSAGE_PINK set ORDER_NO = ORDER_NO + 1 "
			...
			pstmtInsertMessage = conn.prepareStatement("insert into THEME_MESSAGE_PINK values (?,?,?,?,?,?,?,?,?,?,?)");
			...
			pstmtInsertContent = conn.prepareStatement("insert into THEME_CONTENT_PINK values (?,?)");
